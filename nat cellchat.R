...
Cells.sub <- subset(scRNA@meta.data, group=="NAT")
scRNAsub <- subset(scRNA, cells=row.names(Cells.sub))
saveRDS(scRNAsub, file="scRNANAT.rds")

Cells.sub <- subset(scRNA@meta.data, group=="PT")
scRNAsub <- subset(scRNA, cells=row.names(Cells.sub))
saveRDS(scRNAsub, file="scRNAPT.rds")

Cells.sub <- subset(scRNA@meta.data, group=="RT")
scRNAsub <- subset(scRNA, cells=row.names(Cells.sub))
saveRDS(scRNAsub, file="scRNART.rds")
...
par(mfrow = c(1,2))
s.cell <- c( "Epithelial",  "mast cells", "myeloid cells",  "T cells")
...
p <- netVisual_bubble(cellchat, sources.use = c(1,3,4,5,6,7), targets.use = c(2),  comparison = c(1, 2), angle.x = 45)
ggsave("Compare_LR_bubble.pdf", p, width = 25, height = 30)

#气泡图展示上调或下调的配体受体对</p>
p1 <- netVisual_bubble(cellchat, sources.use = c(1,3,4,5,6,7), targets.use = c(2), comparison = c(1, 2), 
                       max.dataset = 2, title.name = "Increased signaling in TIL", angle.x = 45, remove.isolate = T)
p2 <- netVisual_bubble(cellchat, sources.use = c(1,3,4,5,6,7), targets.use = c(2), comparison = c(1, 2), 
